...
public class CartService : ICartService
{
    ...
    {
        if (dto.Quantity <= 0)
            throw new BadRequestException("Quantity 0 veya negatif olamaz.");
            ...
            try
            {
                ...
                if (existingItem is not null)
                {
                    ...
                    if (listing.StockQuantity < newQty)
                        throw new BadRequestException("Sepetteki toplam adet stoğu aşıyor.");
                ...
                if (cartLoaded == null)
                    throw new BadRequestException("Failed to load cart after update");
    ...
    public async Task<CartResponseDto> UpdateCartItemQuantityAsync(int userId, int cartItemId, int quantity)
    {
        ...
        if (quantity <= 0)
        {
            ...
            if (cartAfterRemove == null)
                throw new BadRequestException("Failed to load cart after removing item");
        ...
        if (cart == null)
            throw new BadRequestException("Failed to load cart after quantity update");
        return MapCart(cart);
    }
